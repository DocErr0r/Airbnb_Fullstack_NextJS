...
import { FormData } from '@/app/hotel/create/page'
import Sidebar from '@/app/components/Sidebar'
const page = () => {
    const [hotels,setHotels] = useState<null|HotelInterface[]>(null)
    ...
    const handleUpdateHotel = async(e: React.FormEvent<HTMLFormElement>) => {
      ...
      const res = await fetch(`http://localhost:3000/api/hotel/update/${id}`,{
        method:"POST",
    ...
    const handlerDeleteHotel = async()=>{
        const res = await fetch(`http://localhost:3000/api/hotel/delete/${id}`,{
            method:"DELETE",
...
<table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
            ...
            {hotels.map((hotel)=>{
                return  <tr key={hotel._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                ...
                <td className="flex items-center px-6 py-4">
                    <button  className="font-medium  bg-blue-500 rounded-md  text-white px-5 py-2 hover:underline" onClick={()=>{setEditModal(true),setFormData(hotel),setId(hotel._id)}}>Edit</button>
                    {editModal&&<div id="crud-modal" tabIndex={-1} aria-hidden="true" className=" overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%)] max-h-full bg-black/20">
        <div className="relative p-4 w-full max-w-md mx-auto max-h-full">
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <form onSubmit={handleUpdateHotel} className="p-4 md:p-5" >
              <div className="grid gap-4 mb-4 grid-cols-2">
              <div className="col-span-2">
                  <label htmlFor="title" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Hotel Id</label>
                  <input type="text"  disabled name="title" id="title" className="bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Title" value={hotel._id}  required />
                </div>
...
<div className="col-span-2 sm:col-span-1">
  ...
  <select name="hoteltype" id="hoteltype" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value={formData.hoteltype} onChange={handleChange} required>
    ...
    {/* Add more options as needed */}
              ...
              <button type="submit"   className="w-full  my-3 bg-gradient-to-r text-white py-2 rounded-md px-2 from-red-500 to-pink-500">Update Hotel</button>
            </form>
         
          </div>
        </div>
      </div>} 
                    <button onClick={()=>{setShowModal(true),setId(hotel._id)}}  className="font-medium  ml-2 bg-red-500 rounded-md  text-white px-5 py-2 hover:underline">Delete</button>
...
{/* modal box delete  */}
{showModel&&<div id="popup-modal" tabIndex={-1} className=" overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%)] bg-black/20 max-h-full">
    <div className="relative p-4 w-full max-w-md mx-auto max-h-full ">
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div className="p-4 md:p-5 text-center">
                ...
                <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure you want to delete this hotel ?</h3>
                <button onClick={handlerDeleteHotel} data-modal-hide="popup-modal" type="button" className="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                    Yes, I'm sure
                </button>
